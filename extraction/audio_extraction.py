...
class ServiceRunner(dl.BaseServiceRunner):
    ...
    def extract_audio(self, item: dl.Item, context: dl.Context) -> dl.Item:
        """
        Extract audio from a video item and create a PromptItem with the audio data.
        Outputs 16kHz mono WAV by default (configurable via sample_rate).

        Config keys (customNodeConfig):
            output_dir  : str – remote path for prompt items (default '/audio_prompt_items')
